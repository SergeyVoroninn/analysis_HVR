...
def _inject_and_draw(charts, athlete_id, days, offset):
    for p in charts._plots:
        p._values = []
        for i in range(days):
            ordinal = 738000 + offset + i
            
            # ✅ ИСПРАВЛЕНИЕ: Используем f-string для надежного форматирования даты
            dt = datetime.date.fromordinal(int(ordinal))
            recorded_at = f"{dt:%Y-%m-%d} 12:00:00"
            
            # Теперь кортеж содержит ровно 3 элемента, как в реальной БД
            p._values.append((ordinal, 100.0, recorded_at))
            
        if p._values:
            p._start = datetime.date.fromordinal(int(p._values[0][0]))
            p._end = datetime.date.fromordinal(int(p._values[-1][0]))
        p._draw()
