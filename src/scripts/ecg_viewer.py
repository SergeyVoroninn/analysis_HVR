...
Показывает сырой/отфильтрованный ЭКГ, тахограмму и разности RR.
"""
import os
import sys
import numpy as np
...
class ECGViewer:
    ...
    def __init__(self, root):
        ...
        self.file_info_label.config(text="Нажмите '📁 Открыть файл...' для загрузки данных")
    ...
    def load_file(self, file_path):
        ...
        # Используем готовые RR из файла
    ...
    def plot_data(self):
        ...
        # График 2: Очищенный ЭКГ (БЕЗ отображения пиков)
        ...
        self.axes[1].set_title("Отфильтрованный сигнал ЭКГ", color='white', fontsize=11, fontweight='bold')
        ...
        if len(self.rr_intervals) > 0:
            ...
            self.axes[2].text(0.5, 0.5, "RR-интервалы не найдены", ha='center', va='center', color='white', fontsize=12)
        
        # График 4: Разности RR
        if len(self.rr_intervals) > 1:
            rr_diff = np.diff(self.rr_intervals)
            diff_times = self.rr_times[1:]
            ...
            self.axes[3].text(0.5, 0.5, "Недостаточно данных", ha='center', va='center', color='white', fontsize=12)
    ...
    def update_statistics(self):
        ...
        if self.rr_intervals:
            ...
            large_diffs = np.sum(np.abs(rr_diff) > 100)
