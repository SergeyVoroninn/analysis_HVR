...
def create_ecg(duration_seconds):
    ...
    for row_idx in range(math.ceil(total_samples / samples_per_row)):
        ...
        # Определяем сколько элементов писать в текущую строку
        ...
        for s in range(current_row_samples):
            ...
            if global_idx < 146:
                ...
                baseline = int(-200 + 80 * math.sin(global_idx * 0.05)
...
def create_rr(duration_seconds):
    """
    Генерирует сплошной массив RR-интервалов (мс).
    Соответствует периоду ~115 отсчетов ЭКГ при 130 Гц (~70 уд/мин).
    """
    intervals = []
    accumulated_time_ms = 0
    target_time_ms = duration_seconds * 1000
    while accumulated_time_ms < target_time_ms:
        next_rr = random.randint(860, 910)
...
if __name__ == '__main__':
    # Тест: генерируем одну запись ЭКГ на 10 секунд
